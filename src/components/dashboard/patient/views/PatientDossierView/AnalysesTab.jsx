...
import { fetchConsultationsPatiente, fetchAnalysesConsultation, addAnalyse } from "@/services/gynecologueService";
...
export default function AnalysesTab({ patienteId }) {
  ...
  const [form, setForm] = useState({ type: "", resultat: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    setMsg("");
    try {
      const added = await addAnalyse(selectedConsultId, form, file);
      setAnalyses((a) => [added, ...a]);
      setMsg("Analyse ajoutée avec succès.");
      setShowForm(false);
      setForm({ type: "", resultat: "" });
      setFile(null);
    } catch (err) {
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-2xl border border-pink-100 bg-rose-50/20 p-5 space-y-3">
          <h4 className="font-bold text-gray-900">Nouvelle analyse</h4>
          <Field label="Type d'analyse *" value={form.type} onChange={(v) => setForm((f) => ({ ...f, type: v }))} placeholder="NFS, Glycémie, Échographie..." />
          <div className="space-y-1.5">
            <label className="text-xs font-bold uppercase tracking-wider text-gray-500">Résultat</label>
            <textarea
              rows={3}
              value={form.resultat}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold uppercase tracking-wider text-gray-500">Fichier *</label>
            <input
              type="file"
      ...
      {analyses.length === 0 ? (
        <Empty msg="Aucune analyse pour cette consultation." />
      ) : (
        <div className="space-y-3">
          {analyses.map((a) => (
            <div key={a.id} className="rounded-2xl border border-pink-100 bg-white p-4">
              <div className="flex justify-between">
                <span className="font-bold text-gray-900">{a.type}</span>
                <span className="text-xs text-gray-400">{a.dateAnalyse ? new Date(a.dateAnalyse).toLocaleDateString("fr-FR") : "--"}</span>
              </div>
              <p className="text-sm text-gray-600 mt-1">{a.resultat || "--"}</p>
              {a.fichierNom && <p className="mt-1 text-xs text-rose-600 font-semibold">📎 {a.fichierNom}</p>}
            </div>
          ))}
        </div>
      )}
